...
def get_containers():
    """Get list of all containers. Returns mock containers if Docker is unavailable."""
    ...
    with stats_lock:
        ...
        if MOCK_MODE or not docker_client:
            ...
            try:
                ...
                print(f"Error querying Docker: {e}. Falling back to mock containers.")
                # Fallback to mock containers on error
                for name, details in MOCK_CONTAINERS.items():
                    containers_list.append({
                        "id": details["id"],
                        "name": details["name"],
